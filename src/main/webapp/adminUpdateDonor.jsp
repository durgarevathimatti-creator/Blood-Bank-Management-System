...
<%@ page import="com.codegnan.dao.DonorDao, com.codegnan.model.Donor" %>
<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>Update Donor</title>
<style> 
    body {
...
<div class="container">
    ...
    <%
        ...
        Donor donor = dao.getDonorById(id);
    %>
    <form action="updateDonor" method="post">
        <input type="hidden" name="id" value="<%=donor.getId()%>">
        <input type="text" name="name" value="<%=donor.getName()%>" required><br>
        <input type="text" name="contact" value="<%=donor.getContactNumber()%>" required><br>
        <input type="email" name="email" value="<%=donor.getEmail()%>" required><br>
        <input type="text" name="bloodGroup" value="<%=donor.getBloodGroup()%>" required><br>
        <input type="text" name="address" value="<%=donor.getAddress()%>" required><br>
        <input type="submit" value="Update Donor">
    </form>
    <br><a href="adminViewDonors.jsp">Back to List</a>
</div>
</body></html>